...
def index_file(file, index_name, host="localhost", max_docs=2000000, batch_size=200):
    ...
    # make sure our batch size isn't bigger than our max_docs, else we will never transmit
...
@click.option('--refresh_interval', '-r', default="-1", help="Refresh interval for the index. -1 means disabled. 1s means every second. 30s means every 30 seconds.")
def main(source_dir: str, file_glob: str, index_name: str, workers: int, host: str, max_docs: int, batch_size: int, refresh_interval: str):
    # I believe this is the default max batch size, but need to find docs on that
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=workers) as executor:
        futures = [executor.submit(
            index_file, file, index_name, host, max_docs, batch_size) for file in files]
